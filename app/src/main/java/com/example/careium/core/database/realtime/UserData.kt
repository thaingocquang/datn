...
class UserData : Thread() {
    ...
    override fun run() {
        try {
            ...
            root.child("workout_level").setValue(this.user.workoutLevel)
    ...
    private fun setUserData(snapshot: DataSnapshot, user: User) {
        ...
        if (snapshot.key.toString() == "gender") {
            ...
            when {
                ...
                else -> user.futureGoal = FutureGoal.MaintainWeight
            }
        } else if (snapshot.key.toString() == "workout_level") {
            when {
                snapshot.value.toString() == "Sedentary" -> user.workoutLevel = WorkoutLevel.Sedentary
                snapshot.value.toString() == "SlightlyActive" -> user.workoutLevel = WorkoutLevel.SlightlyActive
                snapshot.value.toString() == "ModeratelyActive" -> user.workoutLevel = WorkoutLevel.ModeratelyActive
                snapshot.value.toString() == "VeryActive" -> user.workoutLevel = WorkoutLevel.VeryActive
                else -> user.workoutLevel = WorkoutLevel.ExtremelyActive
            }
        }
    }
