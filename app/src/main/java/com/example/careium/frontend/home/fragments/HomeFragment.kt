...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private lateinit var KEY_PROGRESS: String
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        preferences = SharedPreferences(requireActivity().applicationContext)
    ...
    private fun observeUserDataCallBackChange() {
        userDataViewModel.mutableUserData.observe(viewLifecycleOwner) { user ->
            if (user != null) {
                ...
                KEY_PROGRESS = user.email

                if (preferences.readProgressCalo(KEY_PROGRESS) == 0) {
                    preferences.writeProgressCalo(KEY_PROGRESS, caloriesTarget)
                    progressVal = caloriesTarget
                } else {
                    progressVal = preferences.readProgressCalo(KEY_PROGRESS)
                }
    ...
    private fun observeNutrition() {
        ...
        nutritionViewModel.mutableNutrition.observe(viewLifecycleOwner) { nutritionList ->
            ...
            if (progressVal - caloriesVal > 0) {
                ...
                preferences.writeProgressCalo(KEY_PROGRESS, progressVal)
            } else progressVal = 0
